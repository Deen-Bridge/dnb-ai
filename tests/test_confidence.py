...
CONFIDENT_ANSWER = (
    "The five daily prayers are Fajr, Dhuhr, Asr, Maghrib and Isha. They are obligatory upon every adult Muslim."
)
...
class TestHedging:
    ...
    def test_advising_a_scholar_is_not_hedging(self):
        ...
        text = (
            "Zakat is due at 2.5% once the nisab is met. Please consult a qualified local scholar for your situation."
        )
...
class TestComputeConfidence:
    ...
    def test_absent_signal_is_skipped_not_treated_as_zero(self):
        ...
        with_zero = compute_confidence(ConfidenceSignals(self_consistency=0.9, citation_verification=0.0))
    ...
    def test_high_stakes_lowers_the_same_evidence(self):
        ...
        stakes = ConfidenceSignals(self_consistency=0.9, expressed_certainty=0.9, is_high_stakes=True)
    ...
    def test_high_stakes_penalty_applies_once(self):
        ...
        assert compute_confidence(signals) == pytest.approx(1.0 - confidence.HIGH_STAKES_PENALTY, abs=1e-4)
    ...
    def test_one_external_signal_lifts_the_ceiling(self):
        ...
        corroborated = compute_confidence(ConfidenceSignals(expressed_certainty=1.0, self_consistency=1.0))
    ...
    def test_score_is_bounded(self):
        for value in (0.0, 0.5, 1.0):
            score = compute_confidence(ConfidenceSignals(self_consistency=value, citation_verification=value))
...
class TestQueueing:
    ...
    def test_queueing_above_the_abstain_band_still_tells_the_user(self, monkeypatch):
        ...
        signals = ConfidenceSignals(self_consistency=0.6, citation_verification=0.6, is_religious=True)
        assessment = assess(signals)
        assert assessment.band is ConfidenceBand.UNCERTAIN
...
class TestAssessAndPolicy:
    def test_low_confidence_religious_answer_abstains_and_queues(self):
        assessment = assess(ConfidenceSignals(self_consistency=0.1, expressed_certainty=0.0, is_religious=True))
    ...
    def test_low_confidence_non_religious_answer_abstains_without_queueing(self):
        assessment = assess(ConfidenceSignals(self_consistency=0.1, is_religious=False))
        assert assessment.abstained is True
        assert assessment.queued is False
    ...
    def test_high_band_answers_unchanged(self):
        assessment = assess(ConfidenceSignals(self_consistency=0.95, citation_verification=1.0))
    ...
    def test_assessment_reports_the_signals_it_used(self):
        assessment = assess(ConfidenceSignals(self_consistency=0.8, expressed_certainty=0.9))
