...
def test_prayer_times_makkah():
    ...
    res = client.get(
        "/prayer-times",
        params={"lat": 21.4225, "lon": 39.8262, "date": "2023-06-15", "method": "MAKKAH", "tz": "Asia/Riyadh"},
    )
...
def test_prayer_times_london_isna():
    ...
    res = client.get(
        "/prayer-times",
        params={"lat": 51.5072, "lon": -0.1276, "date": "2023-01-15", "method": "ISNA", "tz": "Europe/London"},
    )
    assert res.status_code == 200
    data = res.json()
...
def test_prayer_times_high_latitude_summer():
    ...
    res = client.get(
        "/prayer-times",
        params={"lat": 64.1466, "lon": -21.9426, "date": "2023-06-21", "method": "ISNA", "tz": "Atlantic/Reykjavik"},
    )
    assert res.status_code == 200
    data = res.json()
...
def test_invalid_tz():
    res = client.get(
        "/prayer-times", params={"lat": 0, "lon": 0, "date": "2023-01-01", "method": "ISNA", "tz": "Fake/Timezone"}
    )
    assert res.status_code == 400
